...
export default function LandingFloatingChatbot() {
  ...
  // Listen for "open-suro-chat" event from other components
  useEffect(() => {
    const handler = () => setIsOpen(true);
    window.addEventListener("open-suro-chat", handler);
    return () => window.removeEventListener("open-suro-chat", handler);
  }, []);

  return (
    <>
      {/* Chat Bubble */}
      <ChatBubble isOpen={isOpen} onClose={() => setIsOpen(false)} />
